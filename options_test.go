...
func TestParseFlagsRequiresExitNode(t *testing.T) {
	_, err := parseArgs(t)
	require.Error(t, err)
	require.ErrorContains(t, err, "--exit-node")
	require.ErrorContains(t, err, "--experimental-tailcat")
}
...
func TestParseFlagsRejectsTailcatFlagsInTailnetMode(t *testing.T) {
	...
	for name, args := range tests {
		t.Run(name, func(t *testing.T) {
			...
			require.Error(t, err)
			require.ErrorContains(t, err, "--"+name)
			require.ErrorContains(t, err, "requires --experimental-tailcat")
		})
	}
}
...
func TestParseFlagsTailcatDNSMustBeIP(t *testing.T) {
	_, err := parseArgs(t, "--experimental-tailcat", "tcsometoken", "--tailcat-dns", "dns.example.com:53")
	require.Error(t, err)
	require.ErrorContains(t, err, "ip:port")
}
...
func TestParseFlagsTailcatDNSAcceptsIPs(t *testing.T) {
	...
	for _, server := range tests {
		t.Run(server, func(t *testing.T) {
			opts, err := parseArgs(t, "--experimental-tailcat", "tcsometoken", "--tailcat-dns", server)
			require.NoError(t, err)
...
func TestParseFlagsTailcatDNSIgnoredWithLocalDNS(t *testing.T) {
	opts, err := parseArgs(t, "--experimental-tailcat", "tcsometoken", "--local-dns", "--tailcat-dns", "not-an-address")
	require.NoError(t, err)
...
func TestParseFlagsStillRejectsAuthkeyWithOAuth2(t *testing.T) {
	...
	require.Error(t, err)
	require.ErrorContains(t, err, "--authkey cannot be used together with --oauth2")
}
...
func TestParseFlagsRequiresAProxy(t *testing.T) {
	...
	require.Error(t, err)
	require.ErrorContains(t, err, "at least one of --socks-addr and --http-addr")
}
