...
When TCP port forwarding is enabled (the default), the script also starts the proxy with a `--tcp` rule forwarding a local port to the IP-check host, then verifies:
