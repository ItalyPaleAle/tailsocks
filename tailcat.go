...
const (
	// Module path of the tailcat library, used to read back the version this binary was built against
	tailcatModulePath = "github.com/tailscale/tailcat"
	// Reported when the build information does not name a tailcat version, which happens for a binary built outside module mode
	tailcatVersionUnknown = "unknown"
	...
	tailcatTokenEnvVar = "TAILSOCKS_TAILCAT_TOKEN" // #nosec G101 -- Not a credential
...
//
// Whether a UDP dial carries anything is up to the server: forwarding UDP is a capability a server opts into (tailcat.Server.OnUDPForward), which the stock "tailcat serve exit-node" has wired up by default since tailcat v0.7.0; a server running an older tailcat drops UDP in its packet filter before any handler sees it
...
func setupTailcat(ctx context.Context, opts *Options) (*backend, error) {
	...
	slog.Info("Connecting to tailcat server", "clientPublicKey", pubKey, "keyFile", keyPath, "tailcatVersion", tailcatVersion())
...
// tailcatVersion reports the version of the tailcat library this binary was built against
//
// As of date, tailcat promises nothing about its wire format, so a client and a server built from different commits can simply fail to talk to each other, in a way that looks like an unreachable server
// Logging it turns that into something a user can diagnose, by comparing it against what "go version -m" reports for the tailcat binary on the server; tailcat's CLI has no version command of its own
func tailcatVersion() string {
	info, ok := debug.ReadBuildInfo()
	if !ok {
		return tailcatVersionUnknown
	}

	return tailcatVersionFrom(info)
}

// tailcatVersionFrom looks the version up in build information, separated from reading it so it can be tested
func tailcatVersionFrom(info *debug.BuildInfo) string {
	for _, dep := range info.Deps {
		if dep.Path != tailcatModulePath {
			continue
		}

		// A replace directive means a different module is what actually got compiled in
		mod := dep
		if mod.Replace != nil {
			mod = mod.Replace
		}
...
func newTailcatResolver(opts *Options, tunnel *tailcatTunnel) socks5.NameResolver {
	...
	// Queries go through the raw tunnel rather than through tunnelDialer
...
func loadTailcatToken(ctx context.Context, arg string) (tailcat.Addr, error) {
	...
	case strings.Contains(arg, "."):
		// A token is base64-encoded and can never contain a dot, so anything holding one is a DNS name
...
func readTailcatTokenFile(path string) (string, error) {
	...
	read, err := os.ReadFile(path) // #nosec G304 -- not a concern here
	if err != nil {
