...
func startTestSocksProxy(t *testing.T, username string, password string) string {
	t.Helper()

	l, _, err := startSocksProxy(t.Context(), directDialer{}, socks5.DNSResolver{}, "127.0.0.1:0", username, password)
...
func dialThroughSocks5(t *testing.T, proxyAddr string, auth *proxy.Auth, addr string) (io.ReadWriteCloser, error) {
	...
	return d.Dial("tcp", addr) //nolint:wrapcheck
}
...
func TestSocksProxyAuthRejectsMissingCredentials(t *testing.T) {
	...
	proxyAddr := startTestSocksProxy(t, "alice", "hunter2")
...
func TestSocksProxyNoAuthConfiguredAllowsUnauthenticatedClients(t *testing.T) {
	echoAddr := startEchoServer(t)
	proxyAddr := startTestSocksProxy(t, "", "")
